...
class CANAPI(PCANBasic):
    ...
    def __init__(self):
        ...
        self.ext_CANmsg = {'ID': '711', 'DATA': ['02', '10', '03', '00', '00', '00', '00', '00']}
        self.seed_CANmsg = {'ID': '711', 'DATA': ['02', '27', '01', '00', '00', '00', '00', '00']}
    ...
    @time_out
    @func_set_timeout(10)
    def revmanyMsg(self, sorceid, id='719'):
        result_list = []
        sumlen = 0
        msglen = 1
        try:
            ...
            while self.REV_FLAG:
                ...
                if result[0] == 0:
                    try:
                        ...
                        if not msg['ID'] in ['34d', '40a','451']:
                            # self.logger.info('ReadFrame:%s' % str(msg))
                            # self.cmd_map(msg,self.insert_db)
                            self.msg_que.put_nowait(msg)
                            if self.rev_lock.acquire():
                                if self.responseFlag:
                                    self.response_que.put_nowait(msg)
                            else:
                                self.rev_lock.release()
                        else:
                            self.cmsg_que.put_nowait(msg)
                        if resultlist[0] == '10' and not self.responseFlag:
                            newmsg = deepcopy(msg)
                            newmsg['ID'] = '711'
                            newmsg['DATA'] = ['30', '00', '00', '00', '00', '00', '00', '00']
                            self.WriteFrame(newmsg)
    ...
    def auth(self):
        try:
            #切换通道
            self.SendMsgForResp(self.ext_CANmsg, '719')
            time.sleep(0.01)
            #发送keep msg
            #self.SendMsgForResp(keep_CANmsg)
            # 发送鉴权
            seedlist = self.SendMsgForResp(self.seed_CANmsg, '719')
            time.sleep(0.01)
            ...
            rstData = self.SendMsgForResp(DiagDict['key_CANmsg'], '719')
            time.sleep(0.01)
            if len(rstData) > 0:
