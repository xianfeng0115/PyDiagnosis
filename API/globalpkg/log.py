...
class LogSignleton(object):
    ...
    def __new__(cls, log_config):
        ...
        if not hasattr(cls, 'instance'):
            ...
            logdir = cls.instance.log_filename + nowtime.strftime('%Y-%m-%d')
            if not os.path.exists(logdir):
                os.makedirs(logdir)
            cls.instance.log_filename = logdir + '/' + nowtime.strftime('%H-%M-%S') + '_log.txt'
...
logsignleton = LogSignleton('D:/3.gitlab/SAIC-AS28-TBOX_AUTO-TEST/interface_project_for_dev/config/logconfig.conf')
