...
def _check_version(actual):
    from tools.codegenerator import version as required
    if actual != required:
...
def _is_object(obj):
    ...
    from automation import VARIANT
        ...
        if has_name("_NewEnum"):
            class _(partial.partial, self):
                def __iter__(self):
                    ...
                    # IEnumVARIANT
                    from automation import IEnumVARIANT
...
def CoGetClassObject(clsid, clsctx=None, pServerInfo=None, interface=None):
    ...
    if interface is None:
        import server
        interface = server.IClassFactory
    p = POINTER(interface)()
...
################################################################
from _comobject import COMObject
...
from _meta import _coclass_meta
