...
from API.USBI5G import USBIAgree5G
...
class GetAvnData5G(BaseHandler):
...
class telematics_Call_5G(BaseHandler):
...
class RemoteReflash_Check_5G(BaseHandler):
...
class RemoteReflash_Perform_5G(BaseHandler):
...
class Booking_UpdateTime_5G(BaseHandler):
...
#        strip = self.get_argument('serverip')
#        port = self.get_argument('serverport')
#        avn_cmd = self.get_argument('avn_cmd')
#        if strip and port:
#            USBI = USBIAgree5G.get_instances(strip, port)
#            if USBI:
#                USBI.sendMsg(avn_cmd)
#                result = {'result': 0}
#                return_data = json.dumps(result, ensure_ascii=False)
#                self.write(return_data)


class simuRev(BaseHandler):
